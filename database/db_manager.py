...
class DBManager:
    ...
    def delete_user(user_id: int) -> bool:
        ...
        try:
            ...
            print(f"[delete_user] ❌ ERROR: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            try:
                if conn:
    ...
    def update_quote_status(quote_id: int, new_status: str, changed_by: int) -> bool:
        ...
        try:
            ...
            # Actualizar estado y updated_at (para que updated_at refleje la fecha real de aprobación)
            updated_at_now = now_caracas_naive()
            cursor.execute("""
                UPDATE quotes SET status = %s, updated_at = %s WHERE id = %s
            """ if is_postgres else """
                UPDATE quotes SET status = ?, updated_at = ? WHERE id = ?
            """, (new_status, updated_at_now, quote_id))
    ...
    def get_analyst_statistics(analyst_id: int, period: str = 'all') -> Dict[str, Any]:
        ...
        try:
            ...
            # Monto total cotizado (suma de PRECIO USD pago en dólares = total_cost de los ítems)
            cursor.execute(f"""
            ...
            if is_postgres:
                cursor.execute("""
                    SELECT 
                        ...
                        COALESCE(SUM(qi.total_cost), 0) as total_amount
                    FROM quotes q
            ...
            if metric == 'quote_count':
                ...
                cursor.execute(f"""
                    SELECT 
                        u.id as analyst_id,
                        u.full_name as analyst_name,
                        COALESCE(SUM(qi.total_cost), 0) as metric_value
                    FROM users u
